...
**Note:** "Always define `loss_fn` with explicit parameters `(model, x, y)` rather than using closures." This ensures better reusability and compatibility with `mx.compile`.
...
Operations without gradients include indexed assignment, certain integer operations, and those that disrupt graph tracing.

### Gradient Checkpointing for Memory

For very deep models, employ checkpointing to exchange computational cost for reduced memory usage:
